...
function safeUpdateFile(filePath) { const resolved=path.resolve(String(filePath || '')); const safeRoot=`${path.resolve(updatesDownloadDir)}${path.sep}`; if (!resolved.startsWith(safeRoot) || !/\.(?:exe|asar)$/i.test(resolved) || !fs.existsSync(resolved)) throw new Error('ملف التحديث المطلوب غير متاح'); return resolved; }
...
async function installDownloadedAppUpdate(input={}) {
  try {
    await requireUpdateAdministrator();
    const updateFile=safeUpdateFile(input.filePath);
    ...
    const isAsarUpdate=/\.asar$/i.test(updateFile);
    const target=isAsarUpdate ? path.resolve(app.getAppPath()) : process.execPath;
    const previousName=isAsarUpdate ? 'previous-version.asar' : 'previous-version.exe';
    const previous=path.join(verified.folder,previousName);
    if (process.platform!=='win32' || !fs.existsSync(previous)) return { ok:false, error:'ملف التحديث السابق غير متاح للاسترجاع' };
...
async function revealDownloadedUpdate(filePath) {
  ...
  if (!resolved.startsWith(safeRoot) || !/\.(?:exe|asar)$/i.test(resolved) || !fs.existsSync(resolved)) throw new Error('ملف التحديث المطلوب غير متاح');
